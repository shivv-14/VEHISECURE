...
<div class="max-w-4xl mx-auto p-8">
    ...
    <h1 class="text-4xl font-bold text-center mb-8">🚪 Vehicle Exit Scanner</h1>
    ...
    <div class="bg-zinc-900 rounded-3xl p-8 glass shadow-2xl">
        ...
        <div id="previewSection" class="hidden mt-10">
            ...
            <div class="mb-6">
                ...
                <input id="plateInput" type="text" class="w-full bg-zinc-800 text-3xl font-mono p-6 rounded-2xl text-center border border-zinc-700 focus:border-red-500 outline-none" placeholder="TS05AB1234" maxlength="15">
            </div>

            <button onclick="saveExit()" class="bg-red-600 hover:bg-red-700 w-full py-6 text-xl font-bold rounded-3xl">✅ MARK EXIT</button>
        </div>
    </div>
</div>
...
async function runOCR() {
    ...
    try {
        ...
        isProcessing = false;
        btn.disabled = false; 
        btn.textContent = "🔍 Extract Number Plate (OCR)";
    }
}
...
async function saveExit() {
    ...
    try {
        let res = await fetch('process_exit.php', {method: 'POST', body: form});
        let msg = await res.text();
        alert(msg);
        if (msg.includes('success') || msg.includes('recorded') || msg.includes('EXIT')) {
